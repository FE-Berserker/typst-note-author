...
- 改高亮颜色：编辑 `code-theme.tmTheme` 里对应 scope 的色值（scope 写法见
  syntect / TextMate 主题惯例）；
- 底板颜色 / 圆角 / 内边距：`note.typ` 里 `show raw.where(block: true)` 那一条；
- 换回 Typst 默认高亮：删掉 `note.typ` 里的 `set raw(theme: …)` 一行即可。
