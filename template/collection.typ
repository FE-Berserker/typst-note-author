...
// ------------------------------------------------------------
// BEGIN/END 标记之间的 include 列表由 notes_db.py 的 collect 自动维护：
// 一卷收录至多 20 篇**还没进过合集**的笔记（按文件名排序，笔记带日期前缀时
// 排序即卷内顺序），已经编进往期合集的笔记不会再回来——单卷因此不会随着
// 笔记总数越编越厚。要一次编进全部笔记（整套合集），用 collect --full。
// 想自己控制顺序或取舍：删掉下面两行 BEGIN/END 标记，从此完全手工维护，
// collect 不再改动这个区域（出卷前自己把新笔记加全；手工模式下 collect
// 只编译出 PDF、不记收录账）。
// ============================================================
