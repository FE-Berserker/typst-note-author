...
// 全文图形统一样式（figstyle）
// ------------------------------------------------------------
// SYNC: 本文件与 typst-book-author/template/figstyle.typ 的代码相同，仅两处
// 差异：import 的配色字典名（这边 note-colors，那边 book-colors）与个别注释
// 措辞（书/笔记的叫法）。改这里要同步改那边，否则两套模板的图形样式会悄悄
// 漂移；scripts/check_sync.py 可机检。
// ------------------------------------------------------------
